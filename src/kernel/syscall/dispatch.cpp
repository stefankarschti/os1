...
Thread *handle_syscall(TrapFrame *frame)
{
	...
	{
	case os1_sys_write:
		frame->rax = (uint64_t)sys_write(ProcessSyscallContext{
			.frames = &page_frames,
	...
	case os1_sys_read:
	{
		...
		if(try_complete_console_read(page_frames, thread, frame->rsi, (size_t)frame->rdx, read_result))
		{
	...
	case os1_sys_observe:
		frame->rax = static_cast<uint64_t>(sys_observe(ObserveContext{
		...
		return thread;
	case os1_sys_spawn:
		frame->rax = static_cast<uint64_t>(sys_spawn(ProcessSyscallContext{
			.frames = &page_frames,
			.kernel_root_cr3 = g_kernel_root_cr3,
			.write_console_bytes = write_console_bytes,
			.write_cr3 = write_cr3,
	...
	case os1_sys_exec:
		frame->rax = static_cast<uint64_t>(sys_exec(ProcessSyscallContext{
			.frames = &page_frames,
			.kernel_root_cr3 = g_kernel_root_cr3,
			.write_console_bytes = write_console_bytes,
			.write_cr3 = write_cr3,
