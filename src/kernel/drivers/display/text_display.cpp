...
void PresentVgaText(const uint16_t *buffer,
		uint16_t columns,
		uint16_t rows,
		uint16_t cursor_x,
		uint16_t cursor_y)
...
void FramebufferClear(FramebufferTextDisplay &display, uint32_t rgb)
{
	...
	for(uint32_t y = 0; y < display.height; ++y)
	{
		...
		{
			uint8_t *pixel = display.framebuffer
				+ (uint64_t)y * display.pitch_bytes
...
void FramebufferDrawCell(FramebufferTextDisplay &display,
		uint32_t cell_x,
		uint32_t cell_y,
		char character,
		bool inverse)
		...
		uint16_t cursor_y)
{
	...
	const uint32_t origin_y = (display.height > text_height) ? ((display.height - text_height) / 2) : 0;
...
bool InitializeFramebufferTextDisplay(FramebufferTextDisplay &display, const BootFramebufferInfo &framebuffer)
{
	...
	if((0 == framebuffer.physical_address)
		|| (0 == display.width)
		|| (0 == display.height)
