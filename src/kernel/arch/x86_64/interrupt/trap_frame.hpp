...
#define TRAPFRAME_STATIC_ASSERT(name, expr) \
    typedef char trapframe_static_assert_##name[(expr) ? 1 : -1]
...
// Return true when the saved code segment represents ring-3 execution.
[[nodiscard]] inline bool trap_frame_is_user(const TrapFrame& frame)
{
