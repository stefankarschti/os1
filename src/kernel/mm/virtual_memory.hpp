...
public:
	...
	bool map_physical(uint64_t virtual_address, uint64_t physical_address, uint64_t num_pages, PageFlags flags);
	// allocate physical pages, zero them, and map them into a virtual range.
	bool allocate_and_map(uint64_t virtual_address, uint64_t num_pages, PageFlags flags, uint64_t *first_physical = nullptr);
...
private:
	...
	bool walk_to_leaf(uint64_t virtual_address, bool create, bool user_visible, uint64_t **leaf_entry);
