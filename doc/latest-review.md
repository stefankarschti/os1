...
- [2026-05-05 review](2026-05-05-review.md) — source-first review focused on the next steps toward [GOALS.md](../GOALS.md). Reassesses the current code after `kmem` gained real consumers in process/thread registries, ARP cache, device bindings, BAR claims, DMA records, and IRQ route records; calls out stale documentation in [doc/ARCHITECTURE.md](ARCHITECTURE.md), [GOALS.md](../GOALS.md), and this pointer; and prioritizes the descriptor/handle contract, argv/envp, VFS, filesystem-backed exec, first persistent filesystem, credentials, and sequencing prerequisites for networking, SSH, GUI, SMP expansion, POSIX growth, and accelerator work.
